...
class TestNPCSocialNegotiation(unittest.TestCase):
    ...
    def test_casual_talk_never_emulates_regular_success_for_gated_secrets(self):
        """雑談が REGULAR_SUCCESS 扱いにならず、success_level/態度ゲート秘密を漏らさない。"""
        artie = {
            ...
            "secrets": {
                "secret_ref_room_gate": {
                    "content": "参考資料室は関係者以外立ち入り禁止。",
                    "reveal_condition": "True",
                },
                "secret_ruth_blake": {
                    "content": "ルース・ブレイクなら古い記録の不完全さを教える。",
                    "reveal_condition": "success_level >= 2 or relationship == 'cooperative'",
                },
            },
        }
        self.char_mgr.characters["artie_wilmott"] = artie
        result = self.social.resolve_casual_talk("pc_01", "artie_wilmott")
        revealed_ids = {s["id"] for s in result.get("revealed_secrets") or []}
        self.assertIn("secret_ref_room_gate", revealed_ids)
        self.assertNotIn("secret_ruth_blake", revealed_ids)
        self.assertEqual(result["success_level"], 0)
