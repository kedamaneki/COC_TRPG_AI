...
class CharacterManager:
    ...
    def begin_session_stats(self, char_id):
        """シナリオ開始時: 派生プールを初期化し、session_start_san を現在SANで固定する。"""
        char = self.characters.get(char_id)
        if not char or char.get("profile", {}).get("is_npc", False):
            return
        self.initialize_derived_pools(char_id, fill_current=True)
    ...
    # ==========================================
    def load_data(self):
        """2つのファイルを読み込み、メモリ上で結合する"""
        pcs = self._load_json(self.pc_filepath)
        npcs = self._load_json(self.npc_filepath)
    
        self.characters.update(pcs)
        self.characters.update(npcs)
    ...
    def save_data(self):
        """メモリ上のデータを is_npc フラグで判定し、2つのファイルに自動で振り分けて保存する"""
        pcs_to_save = {}
        npcs_to_save = {}

        for char_id, char_data in self.characters.items():
            # is_npcフラグを確認（設定がない場合は安全のためPC扱いにする）
            is_npc = char_data.get("profile", {}).get("is_npc", False)
            
            if is_npc:
                npcs_to_save[char_id] = char_data
            else:
                pcs_to_save[char_id] = char_data

        # それぞれのファイルに保存
        self._save_json(self.pc_filepath, pcs_to_save)
        self._save_json(self.npc_filepath, npcs_to_save)
        print("[システム] データを PC用 と NPC用 のファイルに分割して保存しました。")
