...
if __name__ == "__main__":
    ...
    # 1. 学習用の環境を立ち上げる（学習中は描画しない方が爆速で終わります）
    env = DigitalEcosystemEnv()

    # 2. AIモデルの誕生（PPOアルゴリズムを使用）
    # ※ observation_spaceがDict型（視界＋エネルギー）なので、MultiInputPolicyを指定します
    model = PPO("MultiInputPolicy", env, verbose=1)

    # 3. 学習フェーズ（まずは2万ステップ経験させる）
    print("=== 学習を開始します ===")
    model.learn(total_timesteps=200000)
    print("=== 学習完了！ ===")

    # 4. テストフェーズ（学習したAIの動きを画面で見る）
    env.render_mode = "human"
    obs, info = env.reset()
    
    for step in range(500):
        # AIの脳（モデル）に現在の視界とエネルギーを渡し、最適な行動を予測させる
